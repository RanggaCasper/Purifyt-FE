      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-default bg-elevated">
            ...
            <th class="px-4 py-3 text-left font-medium text-muted">{{ $t('datasetDetail.tableAuthor') }}</th>
            <th class="px-4 py-3 text-left font-medium text-muted">{{ $t('datasetDetail.tableComment') }}</th>
            <th class="px-4 py-3 text-center font-medium text-muted whitespace-nowrap">{{ $t('datasetDetail.tableLabel') }}</th>
        ...
        <tbody class="divide-y divide-default">
          ...
          >
            ...
            <td class="px-4 py-3">
              <p class="text-sm text-highlighted" style="overflow-wrap: anywhere; word-break: break-word">
                {{ comment.comment }}
              </p>
            </td>
            <td class="px-4 py-3">
              <div class="flex flex-col items-center gap-1">
                <LabelBadge v-if="effectiveLabel(comment) !== null" :label="effectiveLabel(comment)!" />
                ...
                >
                  <UIcon name="i-lucide-user-check" class="text-[10px]" />{{ $t('common.manual') }}
                </span>
                <span
                ...
                >
                  <UIcon name="i-lucide-bot" class="text-[10px]" />{{ $t('common.predicted') }}
                </span>
              </div>
            </td>
            <td class="px-4 py-3">
              <div class="flex items-center justify-center gap-1">
                <template v-if="inlineLabelLoading === comment.id">
                  <UIcon name="i-lucide-loader-circle" class="animate-spin text-xs text-muted" />
                </template>
                <template v-else>
                  <UButton
                    size="xs"

...
<template>
  <div v-if="labelingStore.singleResult" class="bg-default border border-default rounded-xl p-5">
    <h3 class="font-medium text-sm text-highlighted mb-4">{{ $t('predict.predictionResult') }}</h3>
    ...
    <div class="space-y-4">
      ...
      <div class="pt-3 border-t border-default">
        <p class="text-xs text-muted mb-1">{{ $t('predict.cleanedText') }}</p>
        <p class="text-sm text-highlighted">{{ labelingStore.singleResult.clean_comment }}</p>
      </div>
    </div>
  </div>
</template>
